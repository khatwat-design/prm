  ...
  return (
    <>
          ...
          {sections.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 w-full text-right ${
                activeSection === id ? 'bg-indigo-500/10 text-indigo-600 dark:bg-indigo-400/20 dark:text-indigo-400 border-r-2 border-indigo-500 dark:border-indigo-400' : ''
              }`}
            >
      ...
      <aside className="hidden md:flex md:flex-col md:fixed md:top-0 md:right-0 md:w-60 md:h-screen md:border-l md:border-slate-200 md:bg-white dark:md:border-slate-700 dark:md:bg-slate-900 md:shadow-lg" dir="rtl">
        <div className="px-5 py-5 border-b border-slate-200 dark:border-slate-700 bg-indigo-500/5 dark:bg-indigo-500/10">
          <span className="text-lg font-bold text-slate-800 dark:text-white">لوحة الأدمن</span>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">إدارة النظام والمستخدمين</p>
        </div>
          ...
          {sections.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 mx-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 ${
                activeSection === id ? 'bg-indigo-500/10 text-indigo-600 dark:bg-indigo-400/20 dark:text-indigo-400 font-medium' : ''
              }`}
            >
              <Icon className="h-5 w-5 shrink-0" />
              {label}
